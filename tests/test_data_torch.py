...
from glyphogen.glyph import NodeGlyph, NODE_GLYPH_COMMANDS, NODE_COMMAND_WIDTH
...
def test_data_integrity(dataset):
    for ix, batch in enumerate(dataset):
        ...
        for i in range(target_sequences.shape[0]):
            ...
            assert torch.argmax(first_command_one_hot) == SOS_INDEX, f"Sequence {i} does not start with SOS"
            second_command_one_hot = target_sequences[i, 1, :NODE_COMMAND_WIDTH]
            assert torch.argmax(second_command_one_hot) in NODE_COMMAND_INDICES, f"Sequence {i} does not have a node command after SOS"
            ...
            if end_of_sequence_index > 0:
                ...
                assert torch.argmax(command_before_eos_one_hot) == Z_INDEX or torch.argmax(command_before_eos_one_hot) in NODE_COMMAND_INDICES, f"Sequence {i} does not have Z or node before EOS"
