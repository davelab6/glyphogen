...
class GlyphDataset(Dataset):
    def __init__(self, font_files, alphabet, is_train=True):
        ...
        self.font_files_train, self.font_files_test = train_test_split(font_files, test_size=0.2, random_state=42)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            glyph_id_one_hot = torch.nn.functional.one_hot(torch.tensor(glyph_index), NUM_GLYPHS).float()
...
class PretrainGlyphDataset(IterableDataset):
    ...
    def _load_and_process_glyph(self, font_file_path, char_ord):
        try:
            ...
            return (
                (torch.from_numpy(raster).float(), torch.from_numpy(target_sequence).long()),
                (torch.from_numpy(true_command).float(), torch.from_numpy(true_coord).float())
            )
        except Exception as e:
