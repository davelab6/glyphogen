...
def calculate_masked_coordinate_loss(y_true_command, y_true_coord, y_pred_coord, arg_counts, delta=10.0):
    ...
    coord_mask = torch.arange(max_args, device=args_needed.device)[None, None, :] < args_needed[..., None]
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim=32):
        ...
        self.deconv1 = nn.ConvTranspose2d(256, 256, 3, stride=2, padding=1, output_padding=1)
        ...
        self.deconv5 = nn.ConvTranspose2d(32, 16, 7, stride=2, padding=3, output_padding=1)
...
class VectorizationGenerator(nn.Module):
    def __init__(self, d_model, latent_dim=32, rate=0.1):
        ...
        self.arg_counts = torch.tensor(list(NODE_GLYPH_COMMANDS.values()), dtype=torch.long)
...
class GlyphGenerator(nn.Module):
    ...
    def __init__(self, num_glyphs, d_model, latent_dim=32, rate=0.1):
        ...
        self.vectorizer = VectorizationGenerator(d_model=d_model, latent_dim=latent_dim, rate=rate)
        self.arg_counts = torch.tensor(list(NODE_GLYPH_COMMANDS.values()), dtype=torch.long)
    ...
    def forward(self, inputs):
        ...
        vectorizer_output = self.vectorizer((generated_glyph_raster, target_sequence_input))
